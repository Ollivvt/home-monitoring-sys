...
# import IFTTT
import requests
LINE_event_name = 'AI_Proj'
LINE_key = 'nEruv9goHkKuC5xtfcz4CKjly_gIJ7I-qd3H6sp_f69'
...
def main():
  ...
  cv2.createTrackbar('Threshold %', 'Object Detecting....', 0, 100, nothing)
  cv2.setTrackbarPos('Threshold %', 'Object Detecting....', 50)
  ...
  while (key_detect==0) :
    ...
    if (times == 1):
      Threshold = cv2.getTrackbarPos('Threshold %','Object Detecting....') / 100
      # 進行影像辨識並將辨識結果存於results
      ...
      for num in range(len(results)) :
        ...
        # if (labels[label_id] == 'wine glass') and (box_left > 320):    # 框選特定類別&指定畫面右半邊綠框
          # 畫框寫值
        ...
        if (labels[label_id] == 'person') and (box_left < 100):    # 指定畫面左半邊紅框
          ...
          r = requests.post(LINE_URL, params={"value1":"現在有人經過",
                                              "value2":labels[label_id],
                                              "value3":round(results[num]['score'],4)})
